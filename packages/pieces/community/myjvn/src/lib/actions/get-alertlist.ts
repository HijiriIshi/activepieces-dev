...
export const getAlertList = createAction({
  ...
  async run(context) {
    ...
    if (datePublished) payload.append('datePublished', datePublished);
    if (dateFirstPublished) payload.append('dateFirstPublished', dateFirstPublished);
    if (cpeName && cpeName.length > 0) payload.append('cpeName', cpeName.join('+'));
    if (ft) payload.append('ft', ft);
